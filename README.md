...
| 項目 | 要件 |
| --- | --- |
| ランナー | kSQL Flow 0.4 系（`npm install` で入ります・MIT） |
| SQL エンジン / MCP | kintone-sql-tools 3.74 系（同上） |
| Node.js | 20.6 以上 |
...
通知メッセージは固定文のみのため「深刻度 + 初動」だけを書きます（例: 条件 1 `【要対応】kSQL Flow バッチ失敗 — error 欄を確認し、対処後に --resume で再実行` / 条件 2 `【要対応】kSQL Flow ジョブ失敗 — error 欄を確認（対処後の再実行は run で可）`）。テンプレートから作ったアプリは**レコードのタイトル = `スクリプト名` が設定済み**なので、通知一覧にジョブ名が並びます（BATCH のタイトルは実行対象ディレクトリ表記。既存アプリは「その他の設定 → レコードのタイトル」で変更）。
...
3. `.env.example` をコピーして `.env` を作り、**このサーバー用の書込可トークン**を貼る（無人サーバーには AI が同居しないため、開発機と違い書込可を `.env` に置く — トークンの露出面はこのディレクトリに閉じる）。共用サーバーでは、このフォルダーに NTFS アクセス権を設定して実行アカウントと管理者以外から `.env` を読めないようにする
4. 疎通確認（ログアプリの定義検査）:
...
- 解説記事:
  ...
  - [【kSQL Flow #4】タスクスケジューラで毎朝動かす — PowerShell が 0.1 秒で無言死する罠つき](https://qiita.com/rex0220/items/d0a66c133edd42ff91c4)（run_batch.bat と定期実行手順の実録）
  ...
  - [【kSQL Flow #13】リラン指示のサーバーレス化で、既存の設計欠陥が見つかった — レビューに 2 度止められて v0.6 が生まれるまで](https://qiita.com/rex0220/items/258ad33ba1e542317602)（チェックボックスリランのサーバーレス化と v0.6 の設計記録）
